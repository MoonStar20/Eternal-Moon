...
namespace Data.DAO
{
    public class PlayerDAO : DAOManager
    {
        ...
        {
            string SqlQuery = "UPDATE `character` SET `deleted`=?delete WHERE `id`=?pid";
            MySqlCommand SqlCommand = new MySqlCommand(SqlQuery, PlayerDAOConnection);
            SqlCommand.Parameters.AddWithValue("?pid", Player.Id);
            SqlCommand.Parameters.AddWithValue("?delete", 1);
            SqlCommand.ExecuteNonQuery();
            try
            {
                SqlCommand.ExecuteNonQuery();
                Log.Info("Player Reqlinquished On PID : {1}", Player.Id);
            }
            catch (Exception ex)
            {
        ...
        public void UpdatePlayer(Player player)
        {
            ...
            + "`AccountName`=?accname,`Level`=?lvl,`Exp`=?exp,`ExpRecoverable`=?exprev,`Mount`=?mount,`UiSettings`=?uiset,`GuildAccepted`=?gaccept,`PraiseGiven`=?praisgive,`LastPraise`=?lastpraise,`Deleted`=?delete`CurrentBankSection`=?bank,`CreationDate`=?credate "
            + "WHERE Id=?id";
            MySqlCommand SqlCommand = new MySqlCommand(SqlQuery, PlayerDAOConnection);
            SqlCommand.Parameters.AddWithValue("?accname", player.AccountName.ToLower());
            SqlCommand.Parameters.AddWithValue("?lvl", player.PlayerLevel);
